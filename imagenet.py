...
parser.add_argument('--imagenet-r-dir', help='Path to ImageNet-R', default="/DATA/imagenet-r/")
parser.add_argument('--imagenet-c-dir', help='Path to ImageNet-C', default="/DATA/imagenet-c/")
...
parser.add_argument('--wd', '--weight-decay', default=5e-4, type=float,
                    ...
                    dest='weight_decay')
parser.add_argument('-pr', '--print-freq', default=10, type=int,
...
parser.add_argument('--A', default=15, type=int, help='Perturbation Intensity of zh(inside the region)')
...
def main_worker(gpu, args):
    ...
    preprocess_aug = transforms.Compose([
        transforms.FCMM(img_size=224, aug_all=args.all_ops, sev=args.aug_severity, mixing_iter=args.k, beta=args.beta, FCMProb=args.p, no_zo=args.no_zo, A=args.A,B=args.B),
#        transforms.StandardAug(img_size=224, aug_all=args.all_ops, sev=args.aug_severity),
#        transforms.FCM(probability = 1.0, no_zo=args.no_zo, A=args.A,B=args.B),
    ...
    train_dataset = AugData(train_data, preprocess, preprocess_aug, args.jsd)
...
def train(train_loader, model, criterion, optimizer, scheduler, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.jsd:
            ...
            bx = images_all.cuda(args.gpu, non_blocking=True)
            by = target.cuda(args.gpu, non_blocking=True)
            
            logits_all = model(bx)
            logits_clean, logits_aug1, logits_aug2 = torch.split(
                logits_all, images[0].size(0))
            loss = criterion(logits_clean, by)
